...
void gotInput(const PM::DataPoints& input, const std::string& sensorFrame, const ros::Time& timeStamp)
{
	try
	{
		...
		PM::TransformationParameters sensorToMapBeforeUpdate = findTransform(sensorFrame, "map", timeStamp, input.getHomogeneousDim());;
...
//		}
        ...
        const PM::TransformationParameters& sensorToMapAfterUpdate = sensorToMapBeforeUpdate;
...
int main(int argc, char** argv)
{
	...
	mapPublisher = n.advertise<sensor_msgs::PointCloud2>("assembled_point_cloud", 2, true);
//	odomPublisher = n.advertise<nav_msgs::Odometry>("icp_odom", 50, true);
	...
	ros::ServiceServer reloadYamlConfigService = pn.advertiseService("reload_yaml_config", reloadYamlConfigCallback);
	ros::ServiceServer saveMapService = pn.advertiseService("save_map", saveMapCallback);
	ros::ServiceServer loadMapService = pn.advertiseService("load_map", loadMapCallback);
	ros::ServiceServer saveTrajectoryService = pn.advertiseService("save_trajectory", saveTrajectoryCallback);
	ros::ServiceServer enableMappingService = pn.advertiseService("enable_mapping", enableMappingCallback);
	ros::ServiceServer disableMappingService = pn.advertiseService("disable_mapping", disableMappingCallback);
