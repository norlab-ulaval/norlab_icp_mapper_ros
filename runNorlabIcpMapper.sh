...
docker run -it \
  ...
  norlab/norlabicpmapper /bin/bash -c "source /opt/ros/humble/setup.bash && source /ros2_ws/install/setup.bash &&source /ros2_ws/src/norlab_icp_mapper_ros/install/setup.bash && ros2 launch norlab_icp_mapper_ros offline_mapper.launch.py bag_path:=/data"
